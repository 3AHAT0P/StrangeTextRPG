...
export const npc1Seed = (options: NPCInteractBuilderOptions): void => {
  const {
  ...
  dataCollection.addLink(spot, {
    ...
    text: `💬 Поговорить с торговцем (${npc1Info.name})`,
    operation: `{{loadMerchantInfo "${npc1Info.id}"}}`,
    type: 'CUSTOM',
    subtype: 'TALK_TO_NPC',
  });
  ...
  const i1 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc1Info.name}]: Привет!`,
  });

  const i2 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc1Info.name}]: Извини, за столь скудный выбор.\n`
  ...
  const i3 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc1Info.name}]: Чего изволишь?`,
  });

  const i4 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc1Info.name}]: К сожалению, у {{actorType player declension="genitive"}} не хватает золота.`,
  });
  ...
  const i6 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc1Info.name}]: Приходи еще :)`,
  });

  const i7 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc1Info.name}]: Хмм... Ладно, для тебя сделаю исключение.\n`
  ...
  const i8 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc1Info.name}]: А, ты еще жив. Ну что ж, уговор дороже денег ;)`,
  });
