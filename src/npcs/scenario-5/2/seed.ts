...
import { npc2Info, poitionExchangeCondition } from './info';
...
export const npc2Seed = (options: NPCInteractBuilderOptions): void => {
  ...
  dataCollection.addLink(spot, {
    ...
    text: `💬 Поговорить с торговцем (${npc2Info.name})`,
    operation: `{{loadMerchantInfo "${npc2Info.id}"}}`,
    type: 'CUSTOM',
    subtype: 'TALK_TO_NPC',
  });
  ...
  const i1 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc2Info.name}]: Привет!`,
  });

  const i2 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc2Info.name}]: Извини, за столь скудный выбор.\n`
  ...
  const i3 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc2Info.name}]: Чего изволишь?`,
  });

  const i4 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc2Info.name}]: К сожалению, у {{actorType player declension="genitive"}} не хватает золота.`,
  });
  ...
  const i6 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc2Info.name}]: Приходи еще :)`,
  });

  const i7 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc2Info.name}]: Хмм... Ладно, для тебя сделаю исключение.\n`
  ...
  const i8 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc2Info.name}]: А, ты еще жив. Ну что ж, уговор дороже денег ;)`,
  });
