...
import { npc4Info, poitionExchangeCondition } from './info';

export const npc4Seed = (options: NPCInteractBuilderOptions): void => {
  const {
  ...
  const i1 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc4Info.name}]: Привет!`,
  });

  const i2 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc4Info.name}]: Извини, за столь скудный выбор.\n`
  ...
  const i3 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc4Info.name}]: Чего изволишь?`,
  });

  const i4 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc4Info.name}]: К сожалению, у {{actorType player declension="genitive"}} не хватает золота.`,
  });
  ...
  const i6 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc4Info.name}]: Приходи еще :)`,
  });

  const i7 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc4Info.name}]: Хмм... Ладно, для тебя сделаю исключение.\n`
  ...
  const i8 = dataCollection.addContainer<InteractionEntity>('Interaction', {
    ...baseInfo,
    text: `💬 [${npc4Info.name}]: А, ты еще жив. Ну что ж, уговор дороже денег ;)`,
  });
