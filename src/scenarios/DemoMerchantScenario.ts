...
interface MarchantProduct {
  internalName: string;
  displayName: string;
  price: number;
}

const merchantGoods = new Map<number, Set<MarchantProduct>>();
...
export class DemoMerchantScenario extends AbstractScenario {
  ...
  private _goods: Set<MarchantProduct> = new Set<MarchantProduct>();
    ...
    if (onDealSuccessAction !== null && onDealFailureAction !== null) {
      ...
      if (exchangeResult) {
        await this._sendTemplateToUser(
          new Template(`⚙️ {{actorType player declension="genitive" capitalised=true}} купил ${choosedGood.displayName.toLowerCase()}`),
          templateContext,
        );
